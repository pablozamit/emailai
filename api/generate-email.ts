...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const { promptData, emailCount, feedbackHistory, apiKey } = req.body;

    if (!promptData || !emailCount || !feedbackHistory || !apiKey) {
        return res.status(400).json({ message: 'Missing required parameters' });
    }

    const prompt = buildMainPrompt(promptData, emailCount, feedbackHistory);
    ...
    try {
        ...
        if (error instanceof SyntaxError) {
            res.status(500).json({ message: "Failed to parse AI response. The response was not valid JSON."});
        }
        res.status(500).json({ message: "Failed to generate email copy from AI." });
    }
}
